...
# Human source uses backticks for internal paths — turn them into real links, not <code>.
PATH_LABELS = {
    "/mashkanta-dira-rishona.html": "משכנתא לדירה ראשונה",
    "/8-sheelot-dira-rishona.html": "8 שאלות לרוכשי דירה ראשונה",
    "/ishur-ekroni.html": "אישור עקרוני",
}
...
ARTICLES = [
    ...
    },
    {
        ...
        "closing": "אם מיחזרתם והחודש כמעט לא זז — או שאתם לפני מיחזור ורוצים לראות גם מה קורה לפוליסה — אפשר לעבור יחד על המספרים שלכם:",
        "disclaimer": "מידע כללי להחלטה מושכלת. אינו ייעוץ ביטוחי מורשה ואינו הבטחת תנאים, אישור או חיסכון. השוואת פוליסות — עם גורם מוסמך לפי הצורך.",
    },
    {
        ...
        "closing": "אם אתם לפני סגירה ורוצים לוודא שיש לכם בחירה אמיתית — או שחתמתם על הראשון בלי להשוות ורוצים להבין מה האפשרויות מעכשיו — אפשר לשים את זה על השולחן בשיחה קצרה:",
        "disclaimer": "מידע כללי להחלטה מושכלת. אינו ייעוץ ביטוחי מורשה ואינו הבטחת תנאים או חיסכון. השוואת פוליסות — עם גורם מוסמך לפי הצורך.",
    },
    {
        ...
        "closing": "אם המשפט «אין לנו מספיק» יושב לכם בבטן — מלאו קודם את טבלת הזמין/מותנה למעלה. אם תרצו שנעזור לכם לתרגם את זה למסגרת עסקה והחזר שאפשר לחיות איתו:",
        "disclaimer": "מידע כללי להחלטה מושכלת. אינו ייעוץ משפטי/מיסוי ואינו הבטחת אישור, תנאים או סכומי מימון. מדיניות בנקים משתנה לפי תיק ועסקה.",
    },
    {
        ...
        "closing": "אם אתם לפני חוזה ויש לכם עקרוני — או שאתם מפחדים לחתום בלי בהירות — אפשר לעבור יחד על מה המסמך באמת אומר לכם, ומה עדיין חסר לפני שמתחייבים:",
        "disclaimer": "מידע כללי להחלטה מושכלת. אינו ייעוץ משפטי ואינו הבטחת אישור, תנאים או לוחות זמנים. כל תיק נבחן לגופו.",
    },
]
    ...
    for m in pattern.finditer(text):
        ...
        if m.group(1) is not None:
            ...
            if path.startswith("/") and path.endswith(".html"):
                ...
                parts.append(
                    f'<a href="{html.escape(href_fix(path), quote=True)}">{html.escape(label)}</a>'
                )
            else:
                parts.append(f"<code>{html.escape(path)}</code>")
        i = m.end()
